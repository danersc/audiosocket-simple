#!/usr/bin/env python3
"""
Script para testar o reconhecimento de voz do Azure Speech SDK com streams de áudio.
Este teste simula mais fielmente como a aplicação principal usa o PushAudioInputStream
para enviar dados SLIN para o reconhecedor.

Uso:
    python test_recognition_stream.py
...
OUTPUT_FILENAME = "audio_capturado_stream.wav"
        ...
        if evt.result.reason == speechsdk.ResultReason.RecognizedSpeech:
            ...
            if self.audio_buffer:
                try:
                    ...
                    filename = f"{DEBUG_DIR}/recognized_stream_{len(self.recognition_results)}.wav"
            ...
            if self.audio_buffer:
                try:
                    audio_data = b"".join(self.audio_buffer)
                    filename = f"{DEBUG_DIR}/nomatch_stream_{int(time.time())}.wav"
                    self.save_audio_to_wav(audio_data, filename)
...
def main():
    ...
    # Configurar parâmetros de segmentação
    ...
    # IMPORTANTE: Usar PushAudioInputStream como na aplicação principal
    # Configurar para SLIN 8kHz
    ...
    print("\n" + "="*60)
    print("Teste de Reconhecimento de Voz com Stream de Áudio")
    print("="*60)
    print(f"SDK Version: {speechsdk.__version__}")
    print(f"Idioma: {speech_config.speech_recognition_language}")
    print(f"Formato: PCM 16-bit, {SAMPLE_RATE}Hz, Mono (SLIN)")
    print(f"Modo: Reconhecimento contínuo com PushAudioInputStream")
    print("="*60)
    ...
    # Inicializar PyAudio para captura do microfone
    p = pyaudio.PyAudio()
    all_audio_chunks = []
    
    # Abrir stream do microfone para captura
    stream = p.open(format=FORMAT,
                    channels=CHANNELS,
                    rate=SAMPLE_RATE,  # Importante: 8kHz para SLIN
                    input=True,
    ...
    try:
        for i in range(0, int(SAMPLE_RATE / CHUNK_SIZE * RECORD_SECONDS)):
            ...
            # Adicionar ao buffer local para diagnóstico
            callbacks.add_audio_chunk(data)
            
            # IMPORTANTE: Enviar para o PushAudioInputStream do Azure
            push_stream.write(data)
            
            # Verificar se tivemos um reconhecimento
            if callbacks.recognized_event.is_set():
                print(f"\nReconhecimento ocorreu! Texto: \"{callbacks.recognition_results[-1]}\"")
                print(f"Áudio salvo em: {DEBUG_DIR}/recognized_stream_{len(callbacks.recognition_results)}.wav")
        ...
        # Fechar o stream para sinalizar o fim do áudio para o Azure
        push_stream.close()
